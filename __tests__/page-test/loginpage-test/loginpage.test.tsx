...
describe('login-page', () => {
    ...
    describe('direct submit on render', () => {
    ...
    describe('correct values for admin', () => {
    ...
    describe('correct values for devotee', () => {
        it('does not render errors when all fields are given for devotee', async () => {
            render(
                <Provider store={store}>
                    <LoginPage />
                </Provider>
            )
            fireEvent.change(screen.getByLabelText('Username'), { target: { value: '2024-Sh-Pu-8' } });
            ...
            await waitFor(() => {
                ...
                expect(screen.queryByLabelText("Password is required")).not.toBeInTheDocument()
                expect(screen.queryByLabelText("Role is required")).not.toBeInTheDocument()
                expect(screen.queryByLabelText("Otp is required")).not.toBeInTheDocument()
            })
        })
    })
})